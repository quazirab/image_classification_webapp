...
from app.dashboard.dynamic_layout import error_message, plot_graph
from app.dashboard.image_processing import (b64_to_pil, img_to_list,
                                            model_wakeup, prediction)
...
def load_callbacks(app):
    ...
    def update_data(content, filename):
        ...
        if filename:
            ...
            if any(
                approved_file_extenstion in filename.lower() for approved_file_extenstion in approved_file_extenstions
            ):
    ...
    @app.callback(Output("row2", "children"), [Input("aggregate_data", "data")])
